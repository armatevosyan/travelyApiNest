...
let EmailService = EmailService_1 = class EmailService {
    ...
    testConnectionOnStartup() {
        setTimeout(() => {
            ...
            this.testConnection()
                .then((isConnected) => {
                if (!isConnected) {
                    this.logger.warn('Email service connection test failed. Email functionality may not work properly.');
                    this.logger.warn('Please check your .env file and ensure EMAIL_USER and EMAIL_PASS are set correctly.');
                }
            })
                .catch((error) => {
                this.logger.error(`Email service connection test failed: ${error instanceof Error ? error.message : String(error)}`, error instanceof Error ? error.stack : undefined);
            });
        }, 1000);
    }
    async testConnection() {
        try {
            ...
            this.logger.error(`Email connection verification failed: ${errorMessage}`, errorStack);
            if (errorMessage.includes('Invalid login')) {
                this.logger.error('Authentication failed. Please check EMAIL_USER and EMAIL_PASS in your .env file.');
            }
            else if (errorMessage.includes('ECONNREFUSED')) {
                this.logger.error('Connection refused. Please check EMAIL_HOST and EMAIL_PORT in your .env file.');
            }
            else if (errorMessage.includes('ETIMEDOUT')) {
                this.logger.error('Connection timeout. Please check your network connection and EMAIL_HOST.');
            }
            return false;
        }
    }
    createTransporter() {
        ...
        this.logger.log(`  Credentials: ${hasCredentials ? 'configured' : 'missing'}`);
        if (!hasCredentials) {
            this.logger.warn('Email credentials are missing. Email functionality will be disabled.');
            this.logger.warn('Please set EMAIL_USER and EMAIL_PASS in your .env file to enable email sending.');
        }
    ...
    async sendEmail(options) {
        ...
        if (!emailConfig?.user || !emailConfig?.pass) {
            this.logger.warn(`Cannot send email to ${options.to}: Email credentials not configured`);
            this.logger.warn('Please set EMAIL_USER and EMAIL_PASS in your .env file to enable email sending.');
            return false;
        }
        try {
            this.logger.debug(`Preparing to send email to ${options.to} with subject: "${options.subject}"`);
            const mailOptions = {
            ...
            this.logger.log(`Email sent successfully to ${options.to} (${duration}ms)`);
            this.logger.debug(`  Message ID: ${messageId}, Accepted: ${accepted}, Rejected: ${rejected}`);
            if (rejected > 0 && result?.rejected) {
                this.logger.warn(`Some recipients were rejected: ${result.rejected.join(', ')}`);
            }
            return true;
        }
        catch (error) {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            const errorStack = error instanceof Error ? error.stack : undefined;
            this.logger.error(`Failed to send email to ${options.to} after ${duration}ms: ${errorMessage}`, errorStack);
            if (errorMessage.includes('Invalid login')) {
                this.logger.error('Authentication failed. Please verify EMAIL_USER and EMAIL_PASS in your .env file.');
            }
            else if (errorMessage.includes('ECONNREFUSED')) {
                this.logger.error('Connection refused. Please check EMAIL_HOST and EMAIL_PORT settings.');
            }
            else if (errorMessage.includes('ETIMEDOUT')) {
                this.logger.error('Connection timeout. Please check your network connection.');
            }
            return false;
        }
    }
